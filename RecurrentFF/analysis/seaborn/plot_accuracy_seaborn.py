...
for idx, file_path in enumerate(adjusted_file_paths):
    ...
    sns.lineplot(x=data["epoch"], y=y_col_upper_bound_smoothed, label=adjusted_labels[idx], color=adjusted_colors[idx], linewidth=3)
    sns.lineplot(x=data["epoch"], y=y_col, color=adjusted_colors[idx], alpha=0.3, linestyle='--', linewidth=3)
