            ...
            with torch.no_grad():
                ...
                for iteration in range(0, iterations):
                    ...
                    if iteration >= lower_iteration_threshold and iteration <= upper_iteration_threshold:
                        # self.inner_layers.reset_activations(not is_test_set)
                        generative_output = torch.zeros(data.shape[1], self.settings.data_config.num_classes).to(self.settings.device.device)
                        for layer in self.inner_layers:
                            if not is_test_set:
                                generative_output += layer.generative_linear_labels(
                        ...
                        assert generative_output.shape[0] == data.shape[1] and generative_output.shape[1] == self.settings.data_config.num_classes
                        # put reconstructed labels through softmax
                        # print(reconstructed_labels[0:3])
                        reconstructed_labels_softmax = F.softmax(generative_output, dim=1)
                        assert generative_output.shape[1] == self.settings.data_config.num_classes
                        assert generative_output.shape[0] == data.shape[1]
                        # argmax to select the label
                        generative_output = torch.argmax(
                            reconstructed_labels_softmax, dim=1)
                        assert generative_output.shape == labels.shape
                        # print(reconstructed_labels[0:10])
                        correct = (generative_output == labels).sum().item()
                        total = data.size(1)
                        # print(correct)
                        # print(total)
