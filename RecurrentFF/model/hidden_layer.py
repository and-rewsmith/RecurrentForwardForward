...
class HiddenLayer(nn.Module):
    ...
    def train_layer(self,  # type: ignore[override]
        ...
        wandb.log({"pos_badness_loss": pos_badness.mean()},
                  step=self.settings.total_batch_count)
        wandb.log({"additional_loss": additional_badness.mean()},
                  step=self.settings.total_batch_count)
        # print(neg_badness[0])
        # print(pos_badness[0])
        # print()
    ...
    def forward(self, mode: ForwardMode, data: torch.Tensor, labels: torch.Tensor, should_damp: bool) -> torch.Tensor:
        ...
        new_activation = F.leaky_relu(summation_act)
        activations_for_mismatch = self.forward_act - neg_contribution_forwards_redo
        activations_for_mismatch_2 = self.backward_act - \
            neg_contribution_backwards_redo
